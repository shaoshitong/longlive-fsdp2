...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs):
        ...
        #     print("DEBUG 238", q.shape, k.shape, v.shape, grid_sizes.shape, freqs.shape)
        x = flash_attention(
...
#     def forward(self, e):
#         print(f"(DEBUG SST) {type(self.modulation)}, {type(e)}")
#         return self.modulation + e

from torch.distributed.tensor import DTensor  # 需要在文件顶部引入
...
class ModulationAdd(nn.Module):
    ...
    def forward(self, e):
        ...
        return modulation.bfloat16() + e.bfloat16()
...
class Head(nn.Module):
    ...
    def forward(self, x, e):
        ...
        assert e.dtype == torch.bfloat16
        # if dist.is_initialized() and dist.get_rank() == 0:
        #     print("DEBUG HEAD", e.shape, self.modulation.shape, x.shape)
        with amp.autocast('cuda', dtype=torch.bfloat16):
